...
public class UserDao {
	...
	public Boolean save(User user) {
		...
		try {
			conn = DBConnection.getConnection();
			String sql = "insert	into user(user_name,nickname,password,register_date,sex,birthday,phone_num) values(?,?,?,?,?,?,?)";
			ps = conn.prepareStatement(sql);
			...
			if (user.getBirthday() == null) {
				...
				ps.setTimestamp(6, new Timestamp(user.getBirthday().getTime()));
			}
	...
	public Boolean update(User user) {
		...
		try {
			conn = DBConnection.getConnection();
			String sql = "update user set user_name=?,nickname=?,password=?,sex=?,birthday=?,phone_num=? where id=?";
			ps = conn.prepareStatement(sql);
			ps.setString(1, user.getUserName());
			ps.setString(2, user.getNickname());
			ps.setString(3, user.getPassword());
			...
			if (user.getBirthday() == null) {
				...
				ps.setTimestamp(5, new Timestamp(user.getBirthday().getTime()));
			}
